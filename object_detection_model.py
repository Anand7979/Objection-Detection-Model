...
def load_image_into_numpy_array(image):#load the image and convert the image data into numpy array
...
TEST_IMAGE_PATH = [os.path.join(PATH_TO_TEST_IMAGES_DIR, 'image{}.jpg'.format(i)) for i in range(1,10)]
IMAGE_SIZE = (12,8)


with detection_graph.as_default():
    with tf.compat.v1.Session(graph=detection_graph) as sess:
        for image_path in TEST_IMAGE_PATH:     # for work on image
            image = Image.open(image_path)     # open the image
            image_np = load_image_into_numpy_array(image)  # create the array of image
            ...
            (boxes, scores, classes, num_detections) = sess.run([boxes, scores, classes, num_detections], feed_dict= {image_tensor:image_np_expanded})  # for running the process

            vis_util.visualize_boxes_and_labels_on_image_array(image_np,
                                                               np.squeeze(boxes),
                                                               np.squeeze(classes).astype(np.int32),   #  for visulation and work on image
                                                               np.squeeze(scores),
                                                               categories_index,
